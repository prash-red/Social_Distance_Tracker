...
def getdepth(objectId,x1,y1, x2,y2):
    ...
    print('avg',objectId,xavg/n,yavg/n,zavg/n)
    return(xavg/n,yavg/n,zavg/n)
    #return(1,1,1)
    ...
    if len(detections) > 0:  						# At least 1 detection in the image and check detection presence in a frame  
        ...
        for detection in detections:				# In this if statement, we filter all the detections for persons only
            ...
            if name_tag == 1:                
                ...
                deptx,depty,deptz = getdepth(objectId,int(x)-25, int(y)-25,int(x) +25, int(y) +25)
                #deptx,depty,deptz = getdepth(xmin, ymin, xmax, ymax)
                #print(xmin, ymin, xmax, ymax,x,y)
                # Append center point of bbox for persons detected.
                centroid_dict[objectId] = (int(x), int(y), xmin, ymin, xmax, ymax,deptx,depty,deptz) # Create dictionary of tuple with 'objectId' as the index center points and bbox
